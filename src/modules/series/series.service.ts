...
export class SeriesService {
  constructor(
    ...
    private readonly detailsRepository: Repository<SeriesDetails>,
  ) {}
  ...
  async addToList(userId: number, dto: CreateSeriesDto): Promise<Series> {
    ...
    if (existing)
      throw new ConflictException('Цей серіал вже є у вашому списку');
  ...
  async remove(userId: number, id: number): Promise<void> {
    ...
    if (result.affected === 0)
      throw new NotFoundException('Серіал не знайдено');
  }
}

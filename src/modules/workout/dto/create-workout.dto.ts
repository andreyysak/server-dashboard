...
export class CreateExerciseDto {
    ...
    @ApiPropertyOptional({ example: 0, description: 'Дистанція в км (для бігу/вело)' })
    ...
    @ApiPropertyOptional({ example: 1, description: 'Порядок вправи у тренуванні' })
...
export class CreateWorkoutDto {
    ...
    @ApiProperty({ enum: WorkoutType, example: WorkoutType.GYM, description: 'Тип тренування' })
    @IsEnum(WorkoutType)
    type: WorkoutType;

    @ApiProperty({ example: '2026-02-24T08:00:00Z', description: 'Дата та час тренування' })
    @IsDateString()
    workout_date: string;

    @ApiPropertyOptional({ example: 45, description: 'Загальна тривалість у хвилинах' })
    @IsInt() @IsOptional() duration_minutes?: number;

    @ApiPropertyOptional({ example: 'Чудове самопочуття, фокус на техніці', description: 'Нотатки' })
